...
// TODO: Cart # header needs to equal the sum of all cartItem quantities; not just the length of the cartitems array. BUT first we need to prevent the duplicate cartItem HTMLs from being created
...
function addToCart(button) {
	if (!button) return;
	// Read the new cart item's info from its HTML attribute, item-info
	const newCartItemInfo = button.parentElement.parentElement.parentElement.querySelector('.item-info');
	
	// Count # times current target cart item title matches titles of existing cart items. Gives us quantity of the item that was in the cart immediately before adding one additional item
	function countExistingNewCartItemsInCart() {
		let count = 0;
		cartItems.forEach(item => {
			if (JSON.stringify(item.title) === JSON.stringify(newCartItemInfo.title)) return count++;
			
			// TODO: Come back to this once we prevent a duplicate cartItemHTML from being created. Will need to use item.quantity attribute once the change is made (given the title will only appear once)
		});
		return count;
	}
	
	// 'count' from addToCart() stored as variable in parent scope
	const existingCount = countExistingNewCartItemsInCart();
	const itemAlreadyExists = Boolean(existingCount).valueOf();
	
	// TODO: NEeds to capture quantity from cart item
	...
	// TODO: IF the item already exists, select cartItems object that matches title of currentTarget object, update quantity attribute in old object, then re-write it to cart. ELSE, create brand new object
	...
	// Final function to run in the two execution functions
	...
	function updateCartIfExistingItem () {
		...
		console.log('increase existing item qty by one');
		
		console.log(cartItems);
		
		// select the matching cart item
		...
		// Update quantity property of object using .syntax:
		// e.g. matchingItem.quantity += 1;
...
function makeCartItemHTML (item) {
	...
	// TODO: In generateCartHTML, read the quantity attribute of newCartItem to set the default <select> value on the individual cart item (on cart.html)
...
function createCheckoutButtonListener() {
	...
	function handleCheckoutButtonClick () {
		// TODO: The event listener for checkout button clicks doesn't do anything yet. This will eventually trigger the checkout flow. We can do this last. For now, just put an alert in here that says "Thanks, please contact sales".
