...
// Initial value declaration; 'let' b/c value will be updated when localStorage is read on future addToCart() events
...
function addToCart(button) {
	...
	// TODO: Use something similar to this for <select> event listeners
			...
			<div class="row">
				...
				<div class="column small">
					<span class="select-item-quantity">
						<span class="item-quantity-options"><select>${selectOptionsHTML}</select></span>
					</span>
				</div>
				<div class="column small">
...
function createUpdateCartQuantityButtonListeners() {	
	...
	function updateQuantity() {
		...
		// 1.) console.log the new value
		// 2.) update the 'selected' option in <select> to that value (remove old selected AND create new one)
		// 3.) Set matchingCartItem.quantity = now-current <select> value. Same manner as in updateCartIfExistingItem()
		
		// 
		// 
		
		// this is the <span class='select-item-quantity'>; however, we need to eventually get to the values inside it

		// 1.) find the 'selected' option and set its innerHTML to remove the 'selected' from it (don't forget the space before 'selected' => ' selected')
		// 2.) set the new 'selected' option -- whatever value the user just picked from the dropddown
		
		
		// remove 'selected' and add it to the new, correct cart quantity
		// --this doesnt work yet--
		const oldSelectedOption = event.currentTarget.querySelector('option value selected');
		console.log(`Old selected option: ${oldSelectedOption}`);
		
		
		// This is the NEW quantity; the cartItem.quantity must be set to this inner value
		const newItemQuantity = parseInt(event.currentTarget.querySelector('option:checked').innerText);		
		console.log(`New item quantity:`, newItemQuantity);
		
			
		// First, find matching item title:
		const matchingItemTitle = event.currentTarget.parentElement.parentElement.parentElement.querySelector('.item-title').innerText;
		console.log(`Matching item title: ${matchingItemTitle}`);
		
		// Then, find matchingCartItem from localStorage
		// const matchingCartItem = 
		const matchingCartItem = cartItems.find(item => item.title === matchingItemTitle);
		console.log(matchingCartItem);
		
		
		// THIS DOESNT WORK YET
		// then set matchingCartItem.quantity = newItemQuantity
		matchingCartItem.quantity = newItemQuantity;
		// to show that the qty is changed:
		console.log(matchingCartItem);
		// do i need to rewrite this to local storage? the quantity is getting updated on the variable i console.log (check the dropdown) -- but it isn't saving on page refresh
		
		// should i use this? or its underlying functions?
		// writeItemsToStorageAndUpdateHeader();
