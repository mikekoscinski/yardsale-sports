	...
	return `
		<div class="card">
			<img 
				...
				alt="${player.name} ${player.year} ${player.issuer} card" 
				tabindex="0" 
				title="${player.name}"
				data-issuer="${player.issuer}"
				data-year="${player.year}"
				data-description="${player.year}, ${player.issuer}"
			>
			<div class="caption">
				<h3>${player.name}</h3>
				<p>${player.issuer}</p>
...
function insertPlayerHTML (dataSource) {
	...
	console.log('Error. Gallery type could not be identified. See insertPlayerHTML() in util.js.')
}
...
export function AutographPlayer(name, team, position) {
	this.name = name;
	this.team = team;
...
export function CardPlayer(name, issuer, year) {
	this.name = name;
	this.issuer = issuer;
	this.year = year;
