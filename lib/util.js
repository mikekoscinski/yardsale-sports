...
function addToCart(button) {
	...
	// TODO: If a cartItem already exists in localStorage, don't create a new object -- just update the existing quantity. 
...
function updateLiveCartTotal () {
	...
	let total = 0;
	cartItems.forEach(item => {
		const priceToAddToTotal = 1 * item.price.replace('$','');
		return total += priceToAddToTotal;
	});
	const liveCartTotal = document.querySelector('.live-cart-total');
	
	// TODO: live-cart-total must = quantity * price, once quantity is a supported attribute.
