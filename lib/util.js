...
// TODO: Will need to check localStorage, similar to the above, to pre-load the total price on the cart page. Store the variable here and update it when specific events are fired. Note that these events should only fire while user is on cart page. Will need some logic like if (isCartPage). Sample event: If quantity is changed (via the select drop-down) while isCartPage === true.
...
function insertCartHTML(dataSource) {	
	...
	// TODO: Insert buyBoxHTML into div class="buy-box" (this is not yet completed)
