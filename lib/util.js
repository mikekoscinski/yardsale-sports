...
export const staticGallery = document.querySelector('.gallery');
...
// TODO: Need to refactor this so that it is just insertPlayerHTML and you pass it 'cards' or 'autograph' parameter

function insertAutographHTML () {
	autographPlayers.forEach(el => {
		const autographHTML = makeAutographPlayerHTML(el);
		staticGallery.insertAdjacentHTML('beforeend', autographHTML);
	});
}
...
export function insertGalleryHTML () {
	if (isAutographGallery) return insertAutographHTML();
	else if (isCardsGallery) return insertCardsHTML();
	console.log('Error. Gallery not found.');
}
