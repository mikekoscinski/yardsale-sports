...
function addToCart(button) {
	...
	// TODO: Check if item is out-of-stock
	
	// Is it better to update data-inventory in real time? Pros: Don't have to reference the cart at all. Cons: Constantly need to update it, as multiple users add and remove items from their carts constantly in real-time. Would then need to re-write the inventory attribute of the Player objects as well. 
	
	// [I prefer this approach] Or should data-inventory stay static until something is ordered? (Prob this one). Then, just have addToCart() reference data-inventory & what's already in the cart?
	
	
	
	
	// const itemIsInStock = Boolean(**cart count of item** === newCartItem.inventory).valueOf();
