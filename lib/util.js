...
// TODO (long-term): Implement Stripe Checkout for checkout flow. For now, just use a "contact sales" alert. (Stripe only charges a rake on payments processed; checkout is free.)
...
function createUpdateCartQuantityButtonListeners() {	
	function updateCartItemQuantity() {		
		...
		// Find the currentTarget <select> menu's associated cartItem from cartItems in localStorage
		const matchingCartItemTitleFromDOM = event.currentTarget.parentElement.parentElement.parentElement.querySelector('.item-title').innerText;
		const matchingCartItemFromLocalStorage = cartItems.find(item => item.title === matchingCartItemTitleFromDOM);
		// If newQty=0, remove item from cartItems and delete its associated parentEl from the DOM
		if (newItemQuantity === 0) {
			const matchingCartItemIndex = cartItems.indexOf(matchingCartItemFromLocalStorage);
			const selectedCartItemParentElement = event.currentTarget.parentElement.parentElement.parentElement;
			if (matchingCartItemIndex > -1) {
				selectedCartItemParentElement.remove();
				cartItems.splice(matchingCartItemIndex, 1);
			}
		}
		matchingCartItemFromLocalStorage.quantity = newItemQuantity;
		writeItemsToStorageAndUpdateHeader();
	}
	const itemQuantityButtons = document.querySelectorAll('.select-item-quantity');
	itemQuantityButtons.forEach(button => 
		button.addEventListener('change', event => updateCartItemQuantity(event.currentTarget))
	);
}
...
function createCheckoutButtonListener() {
	...
	function handleCheckoutButtonClick () {
		alert('Thank you for your business. Please contact our sales department to complete your order.');
	}
