...
(localStorage.getItem('cartItemsKey') !== null) ?
	...
	// TODO: This needs to use .reduce() to add quantity of all cartItems
...
function addToCart(button) {
	if (!button) return;
	// Read the new cart item's info from its item-info HTML attribute
	...
	function updateCartIfExistingItem () {
		...
		// TODO: This works here but not in the parent scope
		// const matchingCartItem = cartItems.find(item => item.title === newCartItemInfo.title);
	...
	function updateCartIfNewItem () {
		...
		// TODO: See if this makes a difference. (Do we even need it if it's a new item? Probably safe to leave as a check)
